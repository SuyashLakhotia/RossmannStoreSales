...
# Run the script with validate as argument for validation (```python xgboostregressor5.py validate```) 
...
if (validate):
    ...
    with open("models/xgboostregressor5-cv.pkl", "wb") as fid:
        pickle.dump(regressor, fid)

    print("Model saved to models/xgboostregressor5-cv.pkl")
    ########### TRAINING COMPLETED ##########

    # Uncomment this block when not training
    # with open("models/xgboostregressor5-cv.pkl", "rb") as fid:
    ...
    result.to_csv("predictions/xgboostregressor5-cv.csv", index=False)

    print("Predictions saved to predictions/xgboostregressor5-cv.csv.")
